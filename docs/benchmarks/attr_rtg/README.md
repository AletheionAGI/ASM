...
- Falharam: os outros 32 gates. A tabela completa está em [`gates.csv`](gates.csv).
...
- [`index.html`](index.html) — dashboard final.
- [`architecture_quality.svg`](architecture_quality.svg) — NMSE/NLL.
- [`governance.svg`](governance.svg) — unsafe, safe-service e coverage.
- [`g_vs_c.svg`](g_vs_c.svg) — trade-off G/C.
- [`seed_differences.svg`](seed_differences.svg) — direção por seed.
