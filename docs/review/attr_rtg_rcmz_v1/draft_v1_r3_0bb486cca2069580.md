...
Every RNG domain uses its own CUDA generator with `seed64=u64be(SHA256(LP("ATTR-RTG-RCMZ-V1")||LP(purpose)||LP(training_seed))[:8])`. Freeze the exact PyTorch/CUDA/driver/GPU hashes. Reductions use ascending canonical indices and a fixed balanced binary tree padded with identity zero. Unordered atomic reductions, fast-math and data-dependent launch counts are prohibited.
...
Peak cap is exactly `20*2^30` bytes. Measure the frozen sequential topology with CUDA synchronization before and after: maximum of allocator allocated/reserved high-water and NVML process use, plus explicit cuBLAS/cuDNN/other workspaces, pinned buffers and CPU/GPU staging. Every transient allocation class must be listed. No retile or fallback is permitted.

Before lock, run the same full-shape synthetic workload twice in clean processes. Payload hashes must be identical. One run must use a prearmed monotonic 20-hour supervisor that writes COMPLETED, CRASH or TIMEOUT without reading metrics. This is observed completion, not formal WCET.
...
6. two clean full-shape synthetic CUDA dry runs with identical hashes, memory and supervisor receipts;
