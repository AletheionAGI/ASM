...
> **STATUS: LOCAL PROTOCOL LOCK — LOCAL-ONLY**
>
> This document defines a simplified benchmark and records its local protocol lock. It does not authorize official data generation, training, calibration or test opening by the agent.
...
Training seeds are exactly `29,43,71,89,107`. There are exactly 20 registered configurations: four arms × five seeds.

The four models are exactly parameter-paired:

| Arm | Model | Trainable parameters | Graph-active parameters | Pairwise mismatch |
|---|---|---:|---:|---:|
| R | ASM-R | 50,000 | 50,000 | 0.000% |
| CM | ASM-CM | 50,000 | 50,000 | 0.000% |
| Z | ASM-Z strict zero-choice | 50,000 | 50,000 | 0.000% |
| T | Transformer | 50,000 | 50,000 | 0.000% |

Every listed parameter must participate naturally in the history-plus-candidate model graph. Padding parameters and synthetic catch-all gradient links are prohibited. The released parameter receipt is computed independently for every arm/seed config.

All arms also use:

- context length 256;
- exact trainable and graph-active parity as listed above;
...
Current state: `LOCAL PROTOCOL LOCK — LOCAL-ONLY`.

The seven local requirements passed and the user explicitly authorized this status-only reseal. Official execution remains assigned to the user in another terminal.
