...
output_root="${OUTPUT_ROOT:-runs/asm_c2_fw_durable_suite}"
device="${DEVICE:-cuda}"
...
for seed in 1 2 3; do
  ...
  .venv/bin/python scripts/train_asm_c2_fw_durable.py \
    --checkpoint "$checkpoint" --output-root "$seed_root" \
    --curriculum "$curriculum" --language-replay-probability "${LANGUAGE_REPLAY:-0.2}" \
    --batch-size "${BATCH_SIZE:-4}" --eval-batches "${EVAL_BATCHES:-32}" \
...
if (( passed_count < 2 )); then
  .venv/bin/python scripts/summarize_asm_c2_fw_durable.py \
    ...
    --output "$output_root/decision.json" --report "$output_root/report.md"
  printf '%s\n' 'Durable memory failed the 2-of-3 curriculum gate; long evaluation blocked.'
  exit 0
fi
...
.venv/bin/python scripts/benchmark_asm_r_long_streaming.py \
  ...
  --lengths 512,4096,32768 --prompt-tokens 64 --mqar-steps 0 \
  --mqar-control-length 40 --mqar-control-threshold 0.8 \
  --mqar-batches "${LONG_EVAL_BATCHES:-128}" --mqar-batch-size 4 \
  --device "$device" --output "$long_root/results.json"

language_root="$output_root/language_regression"
mkdir -p "$language_root"
.venv/bin/python scripts/evaluate_frozen_test.py \
  --family drm --checkpoint "$checkpoint" \
  --manifest data/benchmark_125m_wikipedia/validation/manifest.json --split validation \
  --seq-len 512 --max-tokens 1000000 --batch-size 4 --device "$device" \
...
.venv/bin/python scripts/summarize_asm_c2_fw_durable.py \
  --seed-results "${seed_results[@]}" --long-results "$long_root/results.json" \
...
.venv/bin/python scripts/plot_asm_c2_results.py \
  ...
  --long-results "$long_root/results.json" --output-root "$output_root/charts"

printf 'ASM-C2-FW durable suite complete: %s/report.md\n' "$output_root"
