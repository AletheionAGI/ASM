...
$RunDir = Get-ChildItem -Path $RunRoot -Directory |
    Where-Object { $_.Name -like "drm_125m_real_*_seed_$Seed" } |
    Select-Object -First 1

if ($null -eq $RunDir) {
    throw "DRM seed $Seed run directory not found under $RunRoot"
}
...
if (-not $Checkpoint) {
    throw "No DRM checkpoint found for seed $Seed in $($RunDir.FullName)"
}
